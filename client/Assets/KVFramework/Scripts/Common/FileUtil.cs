...
namespace Core.Resource
{
    public class FileUtil
    {
        ...
        private static string grayDirectory = "";
        ...
        {
            return writeDirectory + grayDirectory + filepath;
        }
        ...
        public static string GetFilePathGrayAll(string filepath)
        {
            ...
            //灰度
            if (FileExist(grayDirectory + filepath, DirectoryType.WritePath))
            {
        ...
        {
            return GetWriteDir() + grayDirectory + filepath;
        }
        ...
        public static string LoadResourceText(string url)
        {
            ...
            {
                Debug.LogWarning(
                    $"[ResourceManager LoadResourceText] Load file error: path = {url}, error = {request.error} )");
                return null;
            }
            return request.downloadHandler.text;
        }
        
        public static byte[] LoadResourceBytes(string url)
        {
            var request = UnityWebRequest.Get(url);
            request.SendWebRequest();
            while (!request.isDone)
            ...
            if (request.error != null)
            {
                Debug.LogWarning($"[ResourceManager LoadResourceBytes] Load file error: path = {url}, error = {request.error} )");
                return null;
            }
        ...
        public static Stream OpenRead(string filePath, bool isOnlyReadPath = false)
        {
            ...
            byte[] bs = LoadResourceBytes(filePath);
            if (bs == null)
            {
                return null;
            }
