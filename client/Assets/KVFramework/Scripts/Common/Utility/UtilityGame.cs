...
public class UtilityGame
{
    ...
    static public void ShaderRecover(GameObject obj)
    {
        ...
        {
            Graphic[] images = obj.GetComponentsInChildren<Graphic>(true);
            for (int j = 0; j < images.Length; j++)
            {
                RecoverMaterialShader(images[j].materialForRendering);
            }
        }
        
        {
            Renderer[] objs = obj.GetComponentsInChildren<Renderer>(true);
            for (int j = 0; j < objs.Length; j++)
            {
                if (objs[j].sharedMaterial == null)
                {
                    continue;
                }
            
                Material[] materials = objs[j].sharedMaterials;
                foreach (var material in materials)
                {
                    if (material != null)
                    {
                        RecoverMaterialShaderQueue(material);
                    }
                }
            }
        }
        ...
        {
            Projector[] objs = obj.GetComponentsInChildren<Projector>(true);
            for (int j = 0; j < objs.Length; j++)
            {
                RecoverMaterialShaderQueue(objs[j].material);
            }
        }
    }


    static private void RecoverMaterialShaderQueue(Material material)
    {
        int que = material.renderQueue;
    ...
    static private void RecoverMaterialShader(Material material)
    {
        if (material)
        {
            material.shader = Shader.Find(material.shader.name);
